...
def candidate_instance_match_diagnostics(
    source_features: torch.Tensor,
    target_features: torch.Tensor,
    source_instance_labels: torch.Tensor,
    target_instance_labels: torch.Tensor,
    ...
    if (
        ...
        zero = _zero(dtype, device)
        return {
            "candidate_instance_match_accuracy": zero,
            "candidate_instance_match_margin": zero,
            "candidate_instance_same_distance": zero,
            "candidate_instance_nearest_other_distance": zero,
            "candidate_instance_hard_match_accuracy": zero,
            "candidate_instance_hard_margin": zero,
            "candidate_instance_hard_same_distance": zero,
            "candidate_instance_nearest_same_group_other_distance": zero,
    ...
    for row in range(source.shape[0]):
        ...
        if bool(target_present.any().item()):
            same_distance = distances[row, target_present].min()
            same_values.append(same_distance)
            candidate_other = other & row_candidates
            if bool(candidate_other.any().item()):
                other_values.append(distances[row, candidate_other].min())
            ...
            if bool(same_group_other.any().item()) and bool(hard_pool.any().item()):
                ...
                hard_hits.append((target_instances[hard_nearest] == source_instances[row]).to(dtype))
                hard_same_values.append(same_distance)
                hard_other_values.append(distances[row, same_group_other].min())
    ...
    if hard_hits:
        ...
        hard_same_distance = torch.stack(hard_same_values).mean()
        hard_other_distance = torch.stack(hard_other_values).mean()
        hard_valid_fraction = torch.tensor(len(hard_hits) / max(1, source.shape[0]), dtype=dtype, device=device)
    else:
        hard_accuracy = zero
        hard_same_distance = zero
        hard_other_distance = zero
        hard_valid_fraction = zero

    return {
        "candidate_instance_match_accuracy": accuracy,
        "candidate_instance_match_margin": other_distance - same_distance,
        "candidate_instance_same_distance": same_distance,
        "candidate_instance_nearest_other_distance": other_distance,
        "candidate_instance_hard_match_accuracy": hard_accuracy,
        "candidate_instance_hard_margin": hard_other_distance - hard_same_distance,
        "candidate_instance_hard_same_distance": hard_same_distance,
        "candidate_instance_nearest_same_group_other_distance": hard_other_distance,
